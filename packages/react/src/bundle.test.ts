...
/**
 * Bundles a snippet the way a user's bundler would: `@pixid/react` resolves
 * to this package's built `dist/` through its own exports map, with its
 * `sideEffects` flag applied. React itself is a peer dependency, so it stays
...
describe('bundling @pixid/react', () => {
  it('stays under 4 KB minified, excluding react', async () => {
    ...
    console.log(`@pixid/react bundle: ${code.length} bytes minified`);
  });
});
