...
- ee2baa7: Add the SVG and PNG renderers that used to live in `@pixid/svg` and `@pixid/png`: `toSvg`, `toSvgDataURL`, `toPng` and `toPngDataURL`. Each takes the icon data from `createIcon()` and an optional scale, and produces exactly the bytes the old packages did:
