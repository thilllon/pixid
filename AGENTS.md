...
build for the same kind of reason: they read `dist/`.

The payoff is that typecheck sees exactly the type surface npm publishes; that is how
the CommonJS declarations mismatch (TS1479 for `"module": "node16"` consumers) showed
up. Two ways to drop the build requirement exist, and neither is used here: TypeScript
project references (`composite` plus `tsc --build`), which still emits declarations but
lets tsc order the work; or pointing `paths` or `customConditions` at each package's
`src`, which type-checks sources instead of the published `.d.ts`, so a broken `exports`
or `types` mapping stops being caught, and vitest and any bundler need the same aliases.
...
root lists is `@pixid/core`, which `assets/generate.ts` imports. Nothing else at the root
needs a `@pixid/*` link: each bundle test imports its own package by name, which resolves
through the package's own `exports` (self-reference), not through the root
`node_modules`.
